...
def main():
  ...
  kaggle_pred_test = dummyAugment(X_test)@w >= 0
  
  test_out = np.concatenate((np.expand_dims(np.array(range(233), dtype=int), axis=1), kaggle_pred_test), axis=1)
  test_out = np.concatenate((header, test_out))
  np.savetxt('test_prediction.csv', test_out, fmt='%s', delimiter=',')
...
def calculateNegativeLogLikelihood(X,y,w):
  ...
  nll = -np.sum(y * np.log(sigma + const) + (1 - y) * np.log(1 - sigma + const))
...
def trainLogistic(X,y, max_iters=max_iters, step_size=step_size):
    ...
    for i in range(max_iters):
        ...
        w_grad = X.T.dot(logistic(X.dot(w)) - y)    #Implementing equation 10 for decreased runtime
...
def dummyAugment(X):
  ...
  aug_X = np.append(np.ones((X.shape[0], 1)), X, axis=1)
